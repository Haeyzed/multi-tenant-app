...
import {
  planPriceIntervalOptions,
  planPriceStatusOptions,
...
function PriceForm({
  ...
  const selected =
    ...
    (draft.currency ? { value: draft.currency, label: draft.currency } : null)
    ...
    <div className="grid grid-cols-2 gap-2 rounded-md border border-dashed p-3 sm:grid-cols-5">
      ...
      <Field>
        ...
        <FieldContent>
          ...
          >
            {planPriceIntervalOptions.map((option) => (
      ...
      <Field>
        ...
        <FieldContent>
          ...
          >
            {planPriceStatusOptions.map((option) => (
              <NativeSelectOption key={option.value} value={option.value}>
                {option.label}
              </NativeSelectOption>
            ))}
